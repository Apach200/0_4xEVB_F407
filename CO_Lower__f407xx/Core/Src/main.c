...
uint8_t Rx_Array[16]  ={8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8};
uint32_t Array_32u[16]={8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8};
...
//GPIO_Blink_Test(GPIOA, GPIO_PIN_7 |GPIO_PIN_6, 25, 33); //for_STM32F4XX_Ali_pcb 7green, 6yellow
...
//CAN_interface_Test();
  ...
  /* USER CODE BEGIN WHILE */
	...
	uint32_t Old_values[8]={0};
	uint32_t New_values[8]={0};
	uint32_t Values_Updated[8]={0};
	uint16_t LLL=0;
	Ticks = HAL_GetTick();
	while(TerminalInterface.gState != HAL_UART_STATE_READY){;}

	  Old_values[0] = OD_PERSIST_COMM.x6040_lowerF_6040;
	  Old_values[1] = OD_PERSIST_COMM.x6041_lowerF_6041;
	  Old_values[2] = OD_PERSIST_COMM.x6042_lowerF_6042;
...
if  (
	 OD_PERSIST_COMM.x6040_lowerF_6040 != Old_values[0]
	)
		 {
		  Old_values[0] = OD_PERSIST_COMM.x6040_lowerF_6040;
...
if  (
	 OD_PERSIST_COMM.x6041_lowerF_6041 != Old_values[1]
	)
		 {
		  Old_values[1] = OD_PERSIST_COMM.x6041_lowerF_6041;
...
if  (
	 OD_PERSIST_COMM.x6042_lowerF_6042 != Old_values[2]
	)
		 {
		  Old_values[2] = OD_PERSIST_COMM.x6042_lowerF_6042;
		  New_values[2] = Old_values[2];
		  Values_Updated[2]=1;
		 }

if  (
	 OD_PERSIST_COMM.x6042_lowerF_6042 != Old_values[0]
	)
		 {
		  Old_values[3] = OD_PERSIST_COMM.x6043_lowerF_6043;
		  New_values[3] = Old_values[3];
		  Values_Updated[3]=1;
		 }
	 ...
	 if(
	   ...
	   )
		{
		  ...
		  LLL= sprintf(
					  Message_to_Terminal,
					  "OD__6040=0x%02X%02X \n\r",
					  (uint16_t)(New_values[0]>>16),
					  (uint16_t)(New_values[0] & 0x0FFFF)
					  );
	 ...
	 if(
	   ...
	   )
		 {
		  ...
		  LLL= sprintf(
					  Message_to_Terminal,
					  "OD__6041=0x%02X%02X \n\r",
					  (uint16_t)(New_values[1]>>16),
					  (uint16_t)(New_values[1] & 0x0FFFF)
					  );
		  HAL_UART_Transmit_IT(
							  &TerminalInterface,
							  (uint8_t*)Message_to_Terminal,
	 ...
	 if(
	   ...
	   )
		 {
		  ...
		  LLL= sprintf(
					  Message_to_Terminal,
					  "OD__6042=0x%02X%02X \n\r",
					  (uint16_t)(New_values[2]>>16),
					  (uint16_t)(New_values[2] & 0x0FFFF)
					  );
		  HAL_UART_Transmit_IT(
							  &TerminalInterface,
							  (uint8_t*)Message_to_Terminal,
	   ...
	   Values_Updated[3]==1
	  )
		 {
		  ...
		  LLL= sprintf(
					  Message_to_Terminal,
					  "OD__6043=0x%02X%02X \n\r",
					  (uint16_t)(New_values[3]>>16),
					  (uint16_t)(New_values[3] & 0x0FFFF)
					  );
		  HAL_UART_Transmit_IT(
							  &TerminalInterface,
							  (uint8_t*)Message_to_Terminal,
...
//	 OD_PERSIST_COMM.x6003_lowerF_VAR32_6003++;
	}
...
void CAN_interface_Test(void)
{
 ...
 //hcan1.Instance->sFIFOMailBox[1].RDLR
 //hcan1.Instance->sFIFOMailBox[1].RDHR
}
...
//	Chip_ID_96bit[2] = HAL_GetUIDw2();
	...
	while(TerminalInterface.gState != HAL_UART_STATE_READY){;}
	HAL_UART_Transmit_IT( &TerminalInterface, (uint8_t*)Message_0, Msg_Length);

	Msg_Length = sizeof(Message_1);
	while(TerminalInterface.gState != HAL_UART_STATE_READY){;}
	HAL_UART_Transmit_IT( &TerminalInterface, (uint8_t*)Message_1, Msg_Length);
	...
	HAL_UART_Transmit_IT( &TerminalInterface, (uint8_t*)Array_for_Messages, Msg_Length);
...
void CO_Init_Return_State(uint16_t Returned_Code)
{
	...
	HAL_UART_Transmit_IT( &TerminalInterface, (uint8_t*)Msg_2_Terminal, Lngth_of_Message);
	while(TerminalInterface.gState != HAL_UART_STATE_READY){;}

switch(Code)
{
case 0:
	HAL_UART_Transmit_IT( &TerminalInterface, (uint8_t*)"  canopen_app_init OK !\n\r\n\r",
													 sizeof"  canopen_app_init OK !\n\r\n\r");break;
case 1:
	HAL_UART_Transmit_IT( &TerminalInterface, (uint8_t*)"Error: Can't allocate memory!\n\r\n\r",
			  	  	  	  	  	  	  	  	  	  	 sizeof"Error: Can't allocate memory!\n\r\n\r");break;
case 2:
	HAL_UART_Transmit_IT( &TerminalInterface, (uint8_t*)"Error: Storage %d ! \n\r\n\r",
			  	  	  	  	  	  	  	  	  	  	 sizeof"Error: Storage %d ! \n\r\n\r");break;
default:
	HAL_UART_Transmit_IT( &TerminalInterface, (uint8_t*)"UNKNOWN code =0x%x \n\r",
