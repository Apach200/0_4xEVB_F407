 ...
 * @param co CANopen object.
 *
 * @param em Emergency object, which is used inside different CANopen objects, usually for error reporting.
 * If NULL, then 'co->em' will be used.
 * if NULL and 'co->CNT_EM' is 0, then function returns with error.
 *
 * @param NMT 	If 'co->CNT_NMT' is 0, this object must be specified,
 * 				If 'co->CNT_NMT' is 1,then it is ignored and can beNULL.
 *  			NMT object is used for retrieving NMT internal state inside CO_process().
 *
 ...
 * @param SDOclientTimeoutTime_ms Default timeout in milliseconds for SDO client, 500 typically.
 * 									SDO client is configured from CO_GTWA_init().
 * @param SDOclientBlockTransfer If true, block transfer will be set in SDO client by default.
 * 									SDO client is configured from by CO_GTWA_init().
 * @param nodeId 	CANopen Node ID (1 ... 127) or 0xFF(unconfigured).
 * 					In the CANopen initialization it is the same as pendingBitRate from CO_LSSinit().
 *  				If it is unconfigured, then some CANopen objects will not be initialized nor processed.
 *
 * @param [out] errInfo   Additional information in case of error, may be NULL.
 * 							errInfo can also be set in noncritical errors, where function returns CO_ERROR_NO.
 * 							For example, if OD parameter contains wrong value.
 *
 * @return CO_ERROR_NO in case of success.
 */
 ...
 * @param od CANopen Object dictionary
 * @param nodeId 	CANopen Node ID (1 ... 127) or 0xFF(unconfigured).
 * 					If unconfigured, then PDO will not be initialized nor processed.
 *
 * @param [out] errInfo Additional information in case of error, may be NULL.
 *
 * @return CO_ERROR_NO in case of success.
 */
CO_ReturnError_t CO_CANopenInitPDO(
									CO_t* co,
									CO_EM_t* em,
									OD_t* od,
									uint8_t nodeId,
 ...
 *
 * @param [out] timerNext_us 			info to OS - maximum delay time after this function should be called next time in [microseconds].
 * 										Value can be used for OS sleep time.
 * 										Initial value must be set to maximum interval time.
 * 										Output will be equal or lower to initial value.
 * 										Calculation is based on various timers which expire in known time.
 * 										Parameter should be used in combination with callbacks configured with CO_***_initCallbackPre() functions.
 * 										Those callbacks should also trigger calling of CO_process() function.
